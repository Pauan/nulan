...
function statementize(x, r) {
  ...
  function expression(x) {
    if (constant.test(x[0])) {
      ...
      while (i < iLen) {
        y = x[i]
        if (constant.test(y[0])) {
          aSub.push(y)
        } else if (y[0] === "var") {
          var u = uniq()
          aSSA.push([u, [y[0]].concat(y.slice(1).map(function (x) {
                       return [x[0], expression(x[1])]
                     })), aSub])
        } else {
          var u = uniq()
          aSSA.push([u, expression(y), aSub])
        }
        ++i
      }
