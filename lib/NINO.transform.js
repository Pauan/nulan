...
// call foo {(call bar {1}) (var {{"qux" 2}}) 3}
    ...
    case "var":
      ...
      if (z.length === 2) {
        ...
        if (y[0] === "if") {
          r.push([x[0], [[z[0]]]])
          if (y[2].length) {
            y[2].push(["=", ["name", z[0]], invert(y[2].pop(), y[2])])
          }
          if (y[3].length) {
            y[3].push(["=", ["name", z[0]], invert(y[3].pop(), y[3])])
          }
          return y
        } else if (y[0] === "return") {
          return invert(y, r)
  ...
  function expression(x, r, first) {
    if (Array.isArray(x)) {
      switch (x[0]) {
      case ",":
        r.push(expression(x[1], r, first))
        return expression(x[2], r, first)
      ...
      case "if":
        x = [x[0], expression(x[1], r, first), block(x[2]), block(x[3])]
        var u = uniq()
        r.push(invert(["var", [[u, x]]], r))
        return ["name", u]

      case "while":
        x = [x[0], expression(x[1], r, first), block(x[2])]
        r.push(x)
        return ["empty"]
      ...
      case "var":
        return invert([x[0], x[1].map(function (x) {
          if (x.length === 1) {
            ...
            return [x[0], expression(x[1], r, first)]
          }
        })], r)

      default:
        return x.map(function (x) {
          return expression(x, r, first)
        })
        /*if (first || isPure(x)) {
          return x
          return x.map(function (x) {
            return expression(x, r, false)
          })
        } else {
          var u = uniq()
          r.push(invert(["var", [[u, x]]], r))
  ...
  function statement(x, r) {
    r.push(invert(expression(x, r, false), r))
    /*console.log(r)
    switch (x[0]) {
    case "return":
      r.push(invert([x[0], expression(x[1], r)], r))
      break
    default:
