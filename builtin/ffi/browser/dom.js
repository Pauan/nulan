...
const make_event_runner = (running) => {
  ...
  return (task) =>
    run_in_thread_pool(pool, on_error, task);
};
...
const event_event = (running, x, event) => {
  ...
  const run = make_event_runner(running);

  return x["addEventListener"](event.b, (e) => run(f({ a: x })), true);
};
...
const event_on_mouse_hover = (running, x, event) => {
  ...
  x["addEventListener"]("mouseover", (e) =>
    run(f({ a: x, b: true })), true);

  // TODO check for descendents
  return x["addEventListener"]("mouseout", (e) =>
    run(f({ a: x, b: false })), true);
};
