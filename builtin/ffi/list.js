...
export const get_index = (index1, length) => {
  ...
  if (index2 >= 0 && index2 < length) {
    ...
    return crash(error_from("invalid index: " + index1));
  }
};
...
export const slice = (a, start1, end1) => {
  ...
  if (start2 === 0 && end2 === length) {
    ...
    return crash(error_from("start index " + start1 + " is greater than end index " + end1));
...
export const chunks = (a, size) => {
  if (size < 1) {
    return crash(error_from("chunk size must be 1 or greater"));
  }
...
export const reduce_right = (init, a, f) => {
