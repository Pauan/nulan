...
export default ({ types: $t }) => {
  return {
    visitor: {
      ...
      ClassMethod: {
        enter: (path, state) => {
          // TODO source line information
          console.warn("Warning: Unused class methods cannot be removed\n    " + state.opts.file + "\n");
        }
      },
      ...
      MemberExpression: {
        enter: (path, state) => {
          ...
          if ($t.isIdentifier(object)) {
            ...
            if (binding != null &&
              ...
              // TODO test this
                ...
                // TODO source line information
                console.warn("Warning: Creating a dynamic namespace for " + name + "\n    " + state.opts.file + "\n");
          ...
          if (!path.node.computed && $t.isIdentifier(property) && property.name.length > 1) {
            // TODO source line information
            console.warn("Warning: Property is longer than 1 character: " + property.name + "\n    " + state.opts.file + "\n");
          }
        }
      },
        ...
        exit: (path, state) => {
          ...
          const top = $t.variableDeclaration("var", [
            $t.variableDeclarator(
              ...
              $t.callExpression($t.memberExpression($t.identifier("Object"), $t.stringLiteral("freeze"), true), [
